...
import LeanQuantumQueries.IndependentMatchingProductOccupancy
...
/-- Ordered-pair collision upper energy.  Every noninjective placement appears
in at least one summand. -/
noncomputable def rawCollisionUpperV (f : S.RawVector) : ℝ :=
  ∑ i, ∑ j ∈ (Finset.univ.erase i),
    S.rawAvg (fun x => S.collisionIndicatorV i j x * (f x) ^ 2)

private theorem rawPlacement_nonemptyV : Nonempty S.RawPlacement := by
  classical
  refine ⟨fun i => ?_⟩
  exact ⟨(S.orbit_nonempty i).choose,
    (S.orbit_nonempty i).choose_spec⟩

private theorem rawPlacement_card_posV :
    0 < (Fintype.card S.RawPlacement : ℝ) := by
  letI := S.rawPlacement_nonemptyV
  exact_mod_cast Fintype.card_pos

/-- Monotonicity of uniform product averaging. -/
theorem rawAvg_monoV {f g : S.RawVector} (h : ∀ x, f x ≤ g x) :
    S.rawAvg f ≤ S.rawAvg g := by
  unfold rawAvg
  apply div_le_div_of_nonneg_right
  · exact Finset.sum_le_sum fun x _ => h x
  · exact (S.rawPlacement_card_posV).le

/-- A finite sum can be moved through product averaging. -/
theorem rawAvg_sumV (F : Fin d → S.RawVector) :
...
/-- Centered pointwise decomposition. -/
theorem synth_apply_eq_mean_add_centeredV (g : S.Coeff)
    (x : S.RawPlacement) :
...
/-- Pointwise Cauchy bound in centered coordinates. -/
theorem synth_sq_le_mean_variance_sumV (g : S.Coeff)
    (x : S.RawPlacement) :
    (S.synth g x) ^ 2 ≤
      2 * (S.totalMean g) ^ 2 +
        2 * (d : ℝ) *
...
theorem q_mul_pairCollisionEnergy_leV
  ...
  have hpoint : ∀ x,
    ...
    exact mul_le_mul_of_nonneg_left
      (S.synth_sq_le_mean_variance_sumV g x) hcoll
  have havg := S.rawAvg_monoV hpoint
  have hrhs :
  ...
  have hrowSum0 : 0 ≤ ∑ k, S.rawNormSq (S.lift k (h k)) := by
...
/-- Collision energy of an additive product-table vector. -/
theorem rawCollisionUpperV_bound
    {q t : ℕ} (g : S.Coeff) (H : S.SectorNumericsV q t) :
    (q : ℝ) * S.rawCollisionUpperV (S.synth g) ≤
  ...
  have hpair : ∀ i, ∀ j ∈ (Finset.univ.erase i),
    ...
    intro i j hj
    exact S.q_mul_pairCollisionEnergy_leV g H i j
      (Finset.ne_of_mem_erase hj)
  unfold rawCollisionUpperV
  rw [Finset.mul_sum]
  have hsum :
    ...
    have hone : ∀ i,
      ...
      calc
        ∑ j ∈ (Finset.univ.erase i),
            (q : ℝ) * S.rawAvg (fun x =>
              S.collisionIndicatorV i j x * (S.synth g x) ^ 2) ≤
            ∑ _j ∈ (Finset.univ.erase i), K :=
          Finset.sum_le_sum fun j hj => hpair i j hj
        _ = ((Finset.univ.erase i).card : ℝ) * K := by
          simp [Finset.sum_const, nsmul_eq_mul]
        _ ≤ (d : ℝ) * K := by
          have hc : (Finset.univ.erase i).card ≤ d :=
            le_trans (Finset.card_erase_le _ _) (by simp)
    ...
    calc
      ...
      _ = (d : ℝ) ^ 2 * K := by
        ...
        ring
  have hsumswap :
      (q : ℝ) * ∑ i, ∑ j ∈ (Finset.univ.erase i),
        S.rawAvg (fun x =>
          S.collisionIndicatorV i j x * (S.synth g x) ^ 2) =
      ∑ i, (q : ℝ) * ∑ j ∈ (Finset.univ.erase i),
        S.rawAvg (fun x =>
          S.collisionIndicatorV i j x * (S.synth g x) ^ 2) := by
    rw [Finset.mul_sum]
  rw [hsumswap]
  have hdNat : d ≤ t := H.coord_le
  have hd : (d : ℝ) ≤ (t : ℝ) := by exact_mod_cast hdNat
  ...
  calc
    _ ≤ (d : ℝ) ^ 2 *
        (16 * (S.totalMean g) ^ 2 +
          16 * (d : ℝ) *
            ∑ k, S.rawNormSq
              (S.lift k (S.centered k (g k)))) := hsum
    _ ≤ 16 * (t : ℝ) ^ 3 *
        ((S.totalMean g) ^ 2 +
          ∑ k, S.rawNormSq
            (S.lift k (S.centered k (g k)))) := by
