import LeanQuantumQueries.IndependentMatchingGoodInnerFinal
import LeanQuantumQueries.IndependentMatchingProductOccupancyFinal
...
/-- A nonnegative sum over legal placements is at most the raw-table sum. -/
theorem sum_legal_le_sum_rawF
...
/-- Occupation by `u` is covered by the coordinate cylinders. -/
theorem legalOccupied_pointwise_leF (u : Fin B) (f : S.RawVector)
    (x : S.Placement) :
  ...
  · rcases hx with ⟨i, hi⟩
    have hiU : i ∈ S.atUIndicesF u := by
      apply (S.mem_atUIndicesF_iff u i).2
      rw [← hi]
      exact (x.1 i).2
...
/-- Legal occupied energy is at most raw-cardinality times the normalized
product occupied upper sum. -/
theorem occupiedEnergy_restrict_le_rawCard_mul_upperF
    {u : Fin B} (c : S.OutsideCoeff u) :
    S.occupiedEnergy u (S.restrict (S.synth c.val)) ≤
      (Fintype.card S.RawPlacement : ℝ) * S.rawOccupiedUpperF c := by
  classical
  ...
  have hlegal :
    ...
    exact Finset.sum_le_sum fun x _ =>
      S.legalOccupied_pointwise_leF u (S.synth c.val) x
  have hraw :
      (∑ x : S.Placement,
        ∑ i ∈ S.atUIndicesF u,
    ...
    apply S.sum_legal_le_sum_rawF
    intro x
    exact Finset.sum_nonneg fun i _ =>
      mul_nonneg (by simp [rawAtU, rawFiber]) (sq_nonneg _)
  have hcardavg :
      (Fintype.card S.RawPlacement : ℝ) * S.rawOccupiedUpperF c =
      ∑ x : S.RawPlacement,
        ∑ i ∈ S.atUIndicesF u,
          S.rawAtU u i x * (S.synth c.val x) ^ 2 := by
...
/-- Occupied energy is subadditive up to factor two. -/
theorem occupiedEnergy_add_le_twoF
    (u : Fin B) (f g : S.Vector) :
    S.occupiedEnergy u (f + g) ≤
      2 * S.occupiedEnergy u f + 2 * S.occupiedEnergy u g := by
  classical
  unfold occupiedEnergy
  have hpoint : ∀ x : S.Placement,
      (if S.Occupied u x then ((f + g) x) ^ 2 else 0) ≤
        2 * (if S.Occupied u x then (f x) ^ 2 else 0) +
          2 * (if S.Occupied u x then (g x) ^ 2 else 0) := by
    intro x
    by_cases hx : S.Occupied u x
  ...
  calc
    (∑ x, if S.Occupied u x then ((f + g) x) ^ 2 else 0) ≤
        ∑ x,
          (2 * (if S.Occupied u x then (f x) ^ 2 else 0) +
            2 * (if S.Occupied u x then (g x) ^ 2 else 0)) :=
...
theorem occupiedEnergy_le_totalEnergyF (u : Fin B) (f : S.Vector) :
